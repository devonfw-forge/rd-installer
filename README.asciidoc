...
For using Windows containers, you will need to start the daemon with `dockerw-start` and get used and adapt scripts to a minor change in the commands (note the `w`):
