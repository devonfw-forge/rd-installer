...
* Support (in addition) for Windows Containers using Docker binary (flag `-WindowsContainers`). You do not need to choose anymore!
...
Using Linux containers with Rancher Desktop is very simple, just make sure that you don't change default containerd runtime, and start using the Docker commands you are used to:
